...
private:
    ...
    List<Persona> *trabajadores;                      // esta es la cola que simula el check-in y check-out de los trabajadores
...
public:
    ...
    unordered_map<string, int> getMaterialesDisponibles();
    ...
    void checkIn(Persona *pTrabajadores);   // hilo, simula el check in de cada trabajador en la cola y lo añade a la cantidad disponible para el proceso actual
    void checkOut(Persona *pTrabajadores);  // hilo, simula el check out de cada trabajador en la cola (todos los del proceso se añaden a la cola cuando se termine para simular que terminaron y se van a ir)
};
